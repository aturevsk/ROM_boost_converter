...
if isfile(latestCp)
    cpFile = latestCp;
elseif isfile(ph2Cp)
    cpFile = ph2Cp;   % Phase 2 done → resume Phase 3
elseif isfile(ph1Cp)
    cpFile = ph1Cp;   % Phase 1 done → resume Phase 2
elseif isfile(bestCp)
    cpFile = bestCp;
else
    cpFile = '';
end
...
if ~isempty(cpFile)
    ...
    fprintf('  Loaded: bestVal=%.6f, epochs=%d, resuming phase %d\n', ...
