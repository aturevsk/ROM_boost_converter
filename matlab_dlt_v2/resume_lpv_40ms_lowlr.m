function resume_lpv_40ms_lowlr(maxHours)
% Resume LPV seed 7: 40ms windows, LR=5e-5, no dlaccelerate.
if nargin < 1, maxHours = 8; end
thisDir=fileparts(mfilename('fullpath')); repoRoot=fileparts(thisDir);
addpath(genpath(repoRoot));
TS=5e-6;SKIP=10;DT=TS*SKIP;GRAD_CLIP=1.0;VAL_FREQ=10;CP_MIN=15;
VAL_IDX=[5,12,14];
...
fprintf('=== Resume LPV: 40ms, LR=5e-5 ===\n');
...
nTotal=numel(csvFiles); profiles_u=cell(nTotal,1);profiles_y=cell(nTotal,1);
for k=1:nTotal,raw=single(readmatrix(fullfile(csvDir,csvFiles(k).name)));
    profiles_u{k}=raw(:,1);profiles_y{k}=raw(:,2:3);end
...
bestNet=net;curLR=5e-5;ext_lr_min=1e-6;
avgG=[];avgSqG=[];phIter=0;noImpExt=0;patCount=0;epCnt=0;
tStart=tic;tLastCp=tic;winMs=40;winSamp=round(winMs*1e-3/TS);
fprintf('\n--- 40ms windows, LR=%.1e ---\n',curLR);
while true
    epCnt=epCnt+1;totalEpochs=totalEpochs+1;tEp=tic;
    allWin={};perm=randperm(nTrain);
    for p=perm,u=train_u{p};y=train_y{p};T=size(u,1);
        nW=max(1,floor(T/winSamp));nU=min(3,nW);
        for s=randperm(nW,nU)-1,si=s*winSamp+1;if si+winSamp-1>T,continue;end
            allWin{end+1,1}=u(si:si+winSamp-1);allWin{end,2}=y(si:si+winSamp-1,:);end,end
    allWin=allWin(randperm(size(allWin,1)),:);
    epochLoss=0;nWin=0;
    for w=1:size(allWin,1)
        x0=dlarray(allWin{w,2}(1,:)');phIter=phIter+1;
        [loss,grads]=dlfeval(@computeLoss,net,x0,dlarray(allWin{w,1}),dlarray(allWin{w,2}),ns,DT,SKIP,0.1,b_lp,a_lp,b_hp,a_hp);
        if isnan(extractdata(loss))||isinf(extractdata(loss)),continue;end
        grads=clipG(grads,GRAD_CLIP);
        [net,avgG,avgSqG]=adamupdate(net,grads,avgG,avgSqG,phIter,curLR);
        epochLoss=epochLoss+extractdata(loss);nWin=nWin+1;
    end
    trainHist(end+1)=epochLoss/max(nWin,1);elapsed=toc(tStart)/3600;epTime=toc(tEp);
    if elapsed>=maxHours,fprintf('  Time limit\n');break;end
    if curLR<=ext_lr_min*1.01,fprintf('  LR floor\n');break;end
    if mod(epCnt,VAL_FREQ)==0||epCnt==1
        [vl,rv,ri]=compVal(net,val_u,val_y,ns,DT,SKIP);
        [fpv,fpi]=compFP(net,val_u,val_y,ns,DT,SKIP);
        valHist(end+1)=vl;imp='';
        if vl<bestVal*(1-1e-4)
            bestVal=vl;bestNet=net;noImpExt=0;patCount=0;imp=' ** BEST **';
            save(fullfile(CP_DIR,'best.mat'),'net','bestNet','bestVal','trainHist','valHist','totalEpochs','ns','-v7.3');
            fprintf('    [cp] best.mat\n');
        else,noImpExt=noImpExt+1;patCount=patCount+1;
            if patCount>=10,curLR=max(curLR*0.5,ext_lr_min);patCount=0;
                fprintf('    LR->%.1e\n',curLR);end,end
        fprintf('  [ep%4d] Train:%.4f|Val:%.4f|FP:V=%.3f iL=%.3f|LR=%.1e|%.1fs|%.2fh|noImp=%d/60%s\n',...
            epCnt,trainHist(end),vl,fpv,fpi,curLR,epTime,elapsed,noImpExt,imp);
        if noImpExt>=60,fprintf('  Plateau\n');break;end
    else,if mod(epCnt,5)==0,fprintf('  [ep%4d] Train:%.4f|%.1fs\n',epCnt,trainHist(end),epTime);end,end
    if toc(tLastCp)>CP_MIN*60
        save(fullfile(CP_DIR,'latest.mat'),'net','bestNet','bestVal','trainHist','valHist','totalEpochs','ns','-v7.3');
        tLastCp=tic;end
end
net=bestNet;[fpv,fpi]=compFP(net,val_u,val_y,ns,DT,SKIP);
save(fullfile(CP_DIR,'final_40ms_lowlr.mat'),'net','bestNet','bestVal','trainHist','valHist','totalEpochs','ns','-v7.3');
fprintf('\n=== Done | %.2fh | bestVal=%.6f | FP:V=%.4f iL=%.4f ===\n',toc(tStart)/3600,bestVal,fpv,fpi);
end
...
x_pred=rk4(net,x0,u_win,ns,dt,skip);T_raw=size(u_win,1);T_sub=size(x_pred,2);
...
if lam_rip>0&&~isempty(b_lp)&&T_sub>20
    ...
    xl=iir(xn',b_lp,a_lp);yl=iir(yn',b_lp,a_lp);xh=iir(xn',b_hp,a_hp);yh=iir(yn',b_hp,a_hp);
    loss=mean((xl(sk+1:end,:)-yl(sk+1:end,:)).^2,'all')+lam_rip*mean((xh(sk+1:end,:)-yh(sk+1:end,:)).^2,'all');
else,loss=mean((xn-yn).^2,'all');end
grads=dlgradient(loss,net.Learnables);
end
function x_traj=rk4(net,x0,u,ns,dt,sk)
T=size(u,1);Ts=floor((T-1)/sk)+1;x=x0;x_traj=x;
for k=1:Ts-1,oi=min((k-1)*sk+1,T);d=u(oi);
    k1=fwd(net,x,d,ns);k2=fwd(net,x+.5*dt*k1,d,ns);
    k3=fwd(net,x+.5*dt*k2,d,ns);k4=fwd(net,x+dt*k3,d,ns);
    x=x+(dt/6)*(k1+2*k2+2*k3+k4);x_traj=cat(2,x_traj,x);end
end
function dxdt=fwd(net,x,d,ns)
...
function [vl,rv,ri]=compVal(net,vu,vy,ns,dt,sk)
tv=0;ti=0;tn=0;
for v=1:numel(vu),xp=rk4(net,dlarray(single(vy{v}(1,:)')),dlarray(single(vu{v})),ns,dt,sk);
    Ts=size(xp,2);idx=1:sk:size(vu{v},1);idx=idx(1:Ts);ys=vy{v}(idx,:)';
    se=(extractdata(xp)-ys).^2;tv=tv+sum(se(1,:));ti=ti+sum(se(2,:));tn=tn+Ts;end
rv=sqrt(tv/tn);ri=sqrt(ti/tn);vl=(tv/ns.x_std(1)^2+ti/ns.x_std(2)^2)/(2*tn);
end
function [fpv,fpi]=compFP(net,vu,vy,ns,dt,sk)
rv=[];ri=[];
for v=1:numel(vu),xp=rk4(net,dlarray(single(vy{v}(1,:)')),dlarray(single(vu{v})),ns,dt,sk);
    Ts=size(xp,2);idx=1:sk:size(vu{v},1);idx=idx(1:Ts);ys=vy{v}(idx,:)';
    rv(end+1)=sqrt(mean((extractdata(xp(1,:))-ys(1,:)).^2));
    ri(end+1)=sqrt(mean((extractdata(xp(2,:))-ys(2,:)).^2));end
fpv=mean(rv);fpi=mean(ri);
end
function g=clipG(g,maxN)
